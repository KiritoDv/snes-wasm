...
static void handleInputKeyboard(int keyCode, bool pressed);
static void handleInputController(SDL_Event *ev);
    ...
    while (SDL_PollEvent(&event)) {
        ...
        case SDL_KEYDOWN: {
            ...
            if ((event.key.keysym.mod & (KMOD_ALT | KMOD_CTRL | KMOD_GUI)) == 0)
            {
                ...
                handleInputKeyboard(event.key.keysym.sym, true);
            }
            break;
        }
        case SDL_KEYUP:
        {
            ...
            handleInputKeyboard(event.key.keysym.sym, false);
            break;
        }
        case SDL_QUIT:
        {
...
int main(int argc, char **argv) {
    ...
    const char* controllerDb = "web/gamecontrollerdb.txt";
    int mappingsAdded = SDL_GameControllerAddMappingsFromFile(controllerDb);
    if (mappingsAdded >= 0) {
        printf("Added SDL game controllers from \"%s\" (%d)\n", controllerDb, mappingsAdded);
    } else {
        printf("Failed add SDL game controller mappings from \"%s\" (%s)\n", controllerDb, SDL_GetError());
    }
    ...
    printf("Found %d joysticks connected\n", num_joysticks);
    joysticks = malloc(sizeof(SDL_Joystick *) * num_joysticks);
...
static int findIndexByController(SDL_Joystick* joystick) {
    for (int i = 0; i < num_joysticks; i++) {
...
static void handleInputController(SDL_Event *e) {
    if (e->type == SDL_JOYDEVICEADDED) {
        SDL_Joystick *joystick = SDL_JoystickOpen(e->jdevice.which);
        if (joystick == NULL) {
        ...
        if (emptySlot == -1) {
            realloc(joysticks, sizeof(SDL_Joystick *) * ++num_joysticks);
            joysticks[num_joysticks - 1] = joystick;
        }
    } else if (e->type == SDL_JOYDEVICEREMOVED) {
        SDL_Joystick *joystick = SDL_JoystickFromInstanceID(e->jdevice.which);
    ...
    } else if (e->type == SDL_JOYBUTTONUP || e->type == SDL_JOYBUTTONDOWN || e->type == SDL_JOYAXISMOTION || e->type == SDL_JOYHATMOTION) {
        int slot = e->jdevice.which;
        if(slot < 0) return;

        printf("Controller %d got an update\n", slot);

        switch (e->type) {
            case SDL_JOYBUTTONUP:
                snes_setButtonState(glb.snes, slot + 1, SDLToSNES(e->jbutton.button), false);
                break;
            case SDL_JOYBUTTONDOWN:
                snes_setButtonState(glb.snes, slot + 1, SDLToSNES(e->jbutton.button), true);
                break;
...
static void handleInputKeyboard(int keyCode, bool pressed) {
    ...
    {
    case SDLK_z:
        snes_setButtonState(glb.snes, 1, 0, pressed);
        break;
    case SDLK_a:
        snes_setButtonState(glb.snes, 1, 1, pressed);
        break;
    case SDLK_RSHIFT:
        snes_setButtonState(glb.snes, 1, 2, pressed);
        break;
    case SDLK_RETURN:
        snes_setButtonState(glb.snes, 1, 3, pressed);
        break;
    case SDLK_UP:
        snes_setButtonState(glb.snes, 1, 4, pressed);
        break;
    case SDLK_DOWN:
        snes_setButtonState(glb.snes, 1, 5, pressed);
        break;
    case SDLK_LEFT:
        snes_setButtonState(glb.snes, 1, 6, pressed);
        break;
    case SDLK_RIGHT:
        snes_setButtonState(glb.snes, 1, 7, pressed);
        break;
    case SDLK_x:
        snes_setButtonState(glb.snes, 1, 8, pressed);
        break;
    case SDLK_s:
        snes_setButtonState(glb.snes, 1, 9, pressed);
        break;
    case SDLK_d:
        snes_setButtonState(glb.snes, 1, 10, pressed);
        break;
    case SDLK_c:
        snes_setButtonState(glb.snes, 1, 11, pressed);
        break;
    }
}
...
static bool checkExtention(const char *name, bool forZip)
{
    ...
    if (forZip)
    {
        if (strcasecmp(name + length - 4, ".zip") == 0)
            return true;
    }
    else
    {
        if (strcasecmp(name + length - 4, ".smc") == 0)
            return true;
        if (strcasecmp(name + length - 4, ".sfc") == 0)
            return true;
    }
    return false;
}
